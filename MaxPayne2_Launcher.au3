...
#AutoIt3Wrapper_Icon=MaxPayne2_icon.ico
#AutoIt3Wrapper_Outfile=MaxPayne2_Launcher.exe
#AutoIt3Wrapper_Compression=4
#AutoIt3Wrapper_UseUpx=n
#AutoIt3Wrapper_UPX_Parameters=-9 --strip-relocs=0 --compress-exports=0 --compress-icons=0
#AutoIt3Wrapper_Res_Description=Max Payne 2 Launcher
...
#pragma compile(FileDescription, 'Max Payne 2 Launcher')
#pragma compile(FileVersion, 1.0.0.47)
#pragma compile(InternalName, 'Max Payne 2 Launcher')
#pragma compile(LegalCopyright, '2014, SalFisher47')
#pragma compile(OriginalFilename, MaxPayne2_Launcher.exe)
#pragma compile(ProductName, 'Max Payne 2 Launcher')
#pragma compile(ProductVersion, 1.0.0.47)
#EndRegion ;**** Pragma Compile ****
; === UniCrack Installer.au3 =======================================================================================================
; Title .........: Max Payne 2 Launcher
; Version .......: 1.0.0.47
; AutoIt Version : 3.3.14.5
; Language ......: English
; Description ...: Max Payne 2 Widescreen Launcher
;				   - based on ThirteenAG's widescreen fix
; Author(s) .....: SalFisher47
; Last Modified .: January 02, 2019
; ==================================================================================================================================
$exe32bit = @ScriptDir & "\MaxPayne2.exe"
$widescreen_fix_ini = @ScriptDir & "\scripts\MP2_widescreen_fix.ini"
$widescreen_fix_asi = @ScriptDir & "\scripts\MP2_widescreen_fix.asi"
...
$RunFirst = IniRead(@ScriptDir & "\MaxPayne2_Launcher.ini", "GAME", "RunFirst", 0)
$cracked = IniRead($widescreen_fix_ini, "EXE", "cracked", "0")
DirCreate(@AppDataCommonDir & "\SalFisher47\WidescreenLauncher")
FileInstall("MaxPayne2_ProgramData.ini", @AppDataCommonDir & "\SalFisher47\WidescreenLauncher\MaxPayne2.ini", 0)
...
$cracked_ProgramData = IniRead(@AppDataCommonDir & "\SalFisher47\WidescreenLauncher\MaxPayne2.ini", "EXE", "cracked", 0)
$CustomizedGame_Bonus = "Bonus Dead Man Walking Chapters"
If Not FileExists($exe32bit) Then
	MsgBox(16, "Max Payne 2 Launcher", "Executable not found:" & @CRLF & "..\MaxPayne2.exe")
	Exit
EndIf
Switch $desktopRatio
	Case 1.33   ; 4:3 aspect ratio, 1024x768
		If not FileExists($widescreen_fix_ini) Then
			...
			FileInstall("d3d9.dll", @ScriptDir & "\d3d9.dll", 0)
			FileInstall("scripts\MP2_widescreen_fix.ini", $widescreen_fix_ini, 0)
			FileInstall("scripts\MP2_widescreen_fix.asi", $widescreen_fix_asi, 0)
		EndIf
		$CustomizedGame_Bonus = "Bonus Dead Man Walking Chapters (4 by 3 aspect ratio)"
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Customized Game", "Customized Game", "REG_SZ", $CustomizedGame_Bonus)
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Video Settings", "Display Bits Per Pixel", "REG_DWORD", Binary(32))
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Video Settings", "Display Height", "REG_DWORD", Binary($desktopY))
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Video Settings", "Display Width", "REG_DWORD", Binary($desktopX))
		If $cracked == 1 Then
			If $cracked_ProgramData == 1 Then
				...
				EndIf
			Else
				FileInstall("MaxPayne2.exe", @ScriptDir & "\MaxPayne2.exe", 1)
				IniWrite($widescreen_fix_ini, "EXE", "cracked", " 1")
				IniWrite(@AppDataCommonDir & "\SalFisher47\WidescreenLauncher\MaxPayne2.ini", "EXE", "cracked", " 1")
				If $RunFirst == 1 Then
					ShellExecute(@AppDataCommonDir & "\SalFisher47\RunFirst\RunFirst.exe", '"' & $exe32bit & '"' & " " & $CmdLineRaw, @ScriptDir, "", @SW_HIDE)
				Else
					ShellExecute($exe32bit, " " & $CmdLineRaw, @ScriptDir)
	...
	Case 1.77   ; 16:9 aspect ratio, 1360x768
		If not FileExists($widescreen_fix_ini) Then
			...
			FileInstall("scripts\MP2_widescreen_fix.asi", $widescreen_fix_asi, 0)
		EndIf
		$CustomizedGame_Bonus = "Bonus Dead Man Walking Chapters (16 by 9 aspect ratio)"
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Customized Game", "Customized Game", "REG_SZ", $CustomizedGame_Bonus)
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Video Settings", "Display Bits Per Pixel", "REG_DWORD", Binary(32))
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Video Settings", "Display Height", "REG_DWORD", Binary($desktopY))
		RegWrite("HKCU\Software\Remedy Entertainment\Max Payne 2\Video Settings", "Display Width", "REG_DWORD", Binary($desktopX))
